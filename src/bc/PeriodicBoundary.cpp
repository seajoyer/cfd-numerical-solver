...
void PeriodicBoundary::Apply(DataLayer& layer, const Mesh& mesh, const Axis axis, const Side side) const {
...
void PeriodicBoundary::Apply(PressureVelocityState& state,
                             const Mesh& mesh,
                             const Axis axis,
    ...
    for (int g = 0; g < ng; ++g) {
        ...
        if (axis == Axis::Y) {
            ...
            const int src_j_p = side == Side::Left ? (j1 - 1 - g) : (j0 + g);
            ...
            const int src_j_vy = side == Side::Left ? (j1 - g) : (j0 + g);
        ...
        const int src_k_p = side == Side::Left ? (k1 - 1 - g) : (k0 + g);
        ...
        const int src_k_wz = side == Side::Left ? (k1 - g) : (k0 + g);
        xt::view(wz, xt::all(), xt::all(), dst_k_wz) =
            xt::view(wz, xt::all(), xt::all(), src_k_wz);
    }
}

void PeriodicBoundary::ApplyPressureVelocityBoundary(PressureVelocityState& state,
...
bool PeriodicBoundary::IsPeriodic() const {
    return true;
}
