...
void NonReflectiveBoundary::Apply2D(DataLayer& layer, int axis, Side side) const {
    ...
    if (axis == 0) {
        // X-axis: fill ghost columns by extrapolating from nearest core column
        for (int j = 0; j < ty; ++j) {
            for (int g = 0; g < pad; ++g) {
                int dst, src;
                if (side == Side::kLeft) {
        ...
        // Y-axis: fill ghost rows by extrapolating from nearest core row
        for (int i = 0; i < tx; ++i) {
            for (int g = 0; g < pad; ++g) {
                int dst, src;
                if (side == Side::kLeft) {
